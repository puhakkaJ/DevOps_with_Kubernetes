...
const Main = () => {
    ...
    const handleDone = async (id) => {
        const { data } = await axios.put(`http://localhost:8082/todos/${id}`)
        console.log(id, data)
        if (data) {
            await axios
                .get('http://localhost:8082/todos')
    ...
    return (
      <div>
        ...
        <ul>
          {todos.map(todo => 
            <li key={todo.id}>{todo.text} {todo.done === true ? <span style={{ color: 'green' }}>DONE</span> : <button onClick={() => handleDone(todo.id)} style={{ color: 'red' }} >Mark done</button>}</li> )}
        </ul>
      </div>
    )
  }
